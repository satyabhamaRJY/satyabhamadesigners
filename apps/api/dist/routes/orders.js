...
const shiprocket_1 = require("../services/shiprocket");
...
router.post('/estimate-shipping', async (req, res) => {
    ...
    try {
        ...
        const estimate = await (0, shiprocket_1.calculateShippingRate)(pincode, totalWeight, maxLength || 10, maxWidth || 10, maxHeight || 5);
...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        // Calculate shipping
        const shippingEstimate = await (0, shiprocket_1.calculateShippingRate)(shippingAddress.pincode, totalWeight, maxLength || 10, maxWidth || 10, maxHeight || 5);
        ...
        const rpOrder = await (0, razorpay_1.createRazorpayOrder)(totalAmount, orderNumber);
        ...
        // Create Order in DB
        const dbOrder = await database_1.prisma.order.create({
            data: {
                orderNumber,
...
router.post('/webhook', async (req, res) => {
    ...
    const isVerified = (0, razorpay_1.verifyWebhookSignature)(JSON.stringify(req.body), signature, webhookSecret);
    if (!isVerified) {
...
router.get('/stats', auth_1.requireAdmin, async (req, res) => {
    try {
        const completedOrders = await database_1.prisma.order.findMany({
            where: {
                paymentStatus: 'COMPLETED'
            },
            select: {
        ...
        const orderCount = await database_1.prisma.order.count();
        const customerCount = await database_1.prisma.customer.count();
